...
public class CoursePdfTheory 
{
    ...
    public CoursePdfTheory(Controll X)
    {
    	TempControll = X;
    }
	...
	private void addContent(Document Doc) throws DocumentException 
	{
		...
		Heading = new Paragraph("Course Wise Tabulation Sheet (Theory)",new Font(Font.FontFamily.TIMES_ROMAN, 10,Font.NORMAL));
		Heading.setAlignment(Element.ALIGN_CENTER);
		Doc.add(Heading);
		...
		//ForcedTable.getDefaultCell().setBorder(Rectangle.NO_BORDER);
	...
	{
		String [] ColumnName = {"SL.NO","Roll No","  Name","Perform (90)","Sec-A (105)","Sec-B (105)","Total Marks (100)","Letter Grade","Grade Point","Remarks"}; 
		PdfPTable table = new PdfPTable(10);
   
		table.setWidthPercentage(100);
    
		float[] Width ={4f,8f,28f,8.4f,8.4f,8.4f,10.5f,7.8f,6.9f,9.6f};
		try 
		  {
		...
		for(int i=this.Counter;i<Capability && this.Counter<this.NumberOfStudents;i++,this.Counter++)
			{
				...
				X = new PdfPCell(new Phrase(DataTransfer.Performance.elementAt(i),CellFont));
				X.setHorizontalAlignment(Element.ALIGN_CENTER);
				X.setVerticalAlignment(Element.ALIGN_CENTER);
				X.setFixedHeight(20f);
				table.addCell(X);
    
				X = new PdfPCell(new Phrase(DataTransfer.SectionA.elementAt(i),CellFont));
				X.setHorizontalAlignment(Element.ALIGN_CENTER);
				X.setVerticalAlignment(Element.ALIGN_CENTER);
				X.setFixedHeight(20f);
				table.addCell(X);
    
				X = new PdfPCell(new Phrase(DataTransfer.SectionB.elementAt(i),CellFont));
				X.setHorizontalAlignment(Element.ALIGN_CENTER);
				X.setVerticalAlignment(Element.ALIGN_CENTER);
				X.setFixedHeight(20f);
				table.addCell(X);
